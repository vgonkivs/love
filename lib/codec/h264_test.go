...
// TestH264Encoder_RequiresStart confirms WriteFrame refuses input
// before Start. The roundtrip path that exercises encode-decode lives
// in TestTSCodec_VideoRoundtrip in ts_codec_test.go.
func TestH264Encoder_RequiresStart(t *testing.T) {
	...
	if err := encoder.WriteFrame(frame); err == nil {
		t.Error("expected error when WriteFrame is called before Start()")
	}
}
...
func TestH264Decoder_RequiresStart(t *testing.T) {
	...
	if _, err := decoder.DecodeH264Frame([]byte{0x00, 0x00, 0x00, 0x01}); err == nil {
